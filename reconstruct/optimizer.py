...
class Optimizer(object):
    ...
    def reconstruct_object(self, t_cam_obj, pts, rays, depth, code=None):
        ...
        """
        """
        :param t_cam_obj: 物体位姿, 物体到相机坐标系的变换
        :param pts: 表面点, 在相机坐标系下的 (M, 3)
        :param rays: 采样出的射线 (N, 3)
        :param depth: 深度值 (K,) 只包含前景像素, K = M for KITTI
        :return: 优化后的物体位姿和形状, 保存为一个字典
        """
        ...
        for e in range(self.num_iterations_joint_optim):
            ...
            # 1. Compute SDF (3D) loss，得到SDF残差，以及关于位姿和形状编码的雅克比
            ...
            # 计算SDF损失
            robust_res_sdf, sdf_loss, _ = get_robust_res(res_sdf, self.b2)  # 计算应用huber范数之后的残差
            # 判断sdf_loss是否不是一个数字
            if math.isnan(sdf_loss):
                return ForceKeyErrorDict(t_cam_obj = None, code = None, is_good = False, loss = loss)

            # 2. Compute Render (2D) Loss，得到渲染残差，以及关于位姿和形状编码的雅克比
            ...
            z = latent_vector.cpu() # 将处理的数据从其他设备拿到cpu上
...
class MeshExtractor(object):
    ...
    def extract_mesh_from_code(self, code):
        ...
        latent_vector = torch.from_numpy(code[:self.code_len]).cuda()
        # get sdf_values
        sdf_tensor = decode_sdf(self.decoder, latent_vector, self.voxel_points)  # "chair_64", code, voxels

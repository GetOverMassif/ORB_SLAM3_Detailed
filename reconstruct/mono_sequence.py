...
class Frame:
    ...
    def get_detections(self):
        ...
        print("Totally detect ", len(bboxes_2d), " objects")
        # TODO: 是否只重建一个物体
        One_Object_Only = True
        # One_Object_Only = False

        # For redwood and freiburg cars, we only focus on the object in the middle
        # 对于 redwood and freiburg cars 数据集，只关注中间的物体
        if (One_Object_Only):
            max_id = np.argmax(masks_2d.sum(axis=-1).sum(axis=-1))
            objectIds = [max_id]
        else:
            objectIds = range(len(masks_2d))

        for n in objectIds:
            mask_pre = masks_2d[n, ...].astype(np.float32) * 255.
            bbox_pre = bboxes_2d[n, ...]

            non_surface_pixels = self.pixels_sampler(bbox_pre, mask_pre.astype(np.bool8))
            ...
            instance.bbox = bbox_pre
            instance.mask = mask_pre

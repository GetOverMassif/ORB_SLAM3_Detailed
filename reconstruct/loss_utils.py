...
def decode_sdf(decoder, lat_vec, x, max_batch=64**3):
    ...
    with torch.no_grad():
        while head < num_samples:
            # 批处理查询点,生成对应份数的lat_vec,将lat_vec与xyz连接在一起,查询到一批对应的SDF值
...
def get_batch_sdf_jacobian(decoder, lat_vec, x, out_dim=1):
    """
    :param decoder: DeepSDF Decoder
    :param lat_vec: torch.Tensor (code_len,), latent code 隐式编码向量
    :param x: torch.Tensor (N, 3), query position 查询位置
    :param out_dim: int, output dimension of a single input 单一输入的输出维度
    :return: batched Jacobian (N, out_dim, code_len + 3) 成批的雅克比矩阵
    """
    n = x.shape[0]
...
def get_robust_res(res, b):
    """
    :param res: residual vectors 残差向量
    :param b: threshold 阈值
    :return: residuals after applying huber norm 应用huber范数之后的残差
    """
    # print(res.shape[0])
    res = res.view(-1, 1, 1)
    ...
    # print("\nrobust_res = \n", robust_res)
    # print("\nloss = \n", loss)
    return robust_res, loss, w  # 损失loss 是 残差res 的平方均值

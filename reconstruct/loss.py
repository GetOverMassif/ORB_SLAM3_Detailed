...
def compute_sdf_loss(decoder, pts_surface_cam, t_obj_cam, latent_vector):
    ...
    :return: Jacobian wrt pose (N, 1, 7), Jacobian wrt shape code (N, 1, code_len), error residuals (N, 1, 1)
             关于位姿的雅克比 (N,1,7), 关于形状编码的雅克比 (N,1,code_len), 误差残差 (N,1,1)
    """
    # (n_sample_surface, 3)
    # 表面点在物体坐标系的坐标
    # print("\npts_surface_cam[..., None, :] = \n", pts_surface_cam[..., None, :])
    # print("\nt_obj_cam.cuda()[:3, :3] = \n", t_obj_cam.cuda()[:3, :3])
    # print("\nt_obj_cam.cuda()[:3, :3].sum(-1) = \n", t_obj_cam.cuda()[:3, :3].sum(-1))
    # print("\nt_obj_cam.cuda()[:3, 3] = \n", t_obj_cam.cuda()[:3, 3])
    ...
    # SDF term Jacobian, d(e)/d(x_o)
    de_dxo = de_di[..., -3:]
    # Jacobian for pose, d(x_o)/d(T_oc)
